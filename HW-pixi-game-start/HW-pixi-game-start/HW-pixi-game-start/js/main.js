...
function gameLoop(){
  ...
  // enable triple fire
  if (score >= 5) {
    tripleFire = true; // Enable triple fire
  }
  else {
    tripleFire = false;
  }
...
function end() {
  ...
  // Clear remaining circles, bullets, and explosions
...
function fireBullet() {
  ...
  if (tripleFire) {
      // Fire three bullets: one straight, one slightly to the left, and one slightly to the right
      let bullet1 = new Bullet(0xffffff, ship.x, ship.y);
      let bullet2 = new Bullet(0xffffff, ship.x - 10, ship.y);
      let bullet3 = new Bullet(0xffffff, ship.x + 10, ship.y);

      bullets.push(bullet1, bullet2, bullet3);
      gameScene.addChild(bullet1, bullet2, bullet3);
  } else {
  ...
  shootSound.play(); // Play sound once
}
